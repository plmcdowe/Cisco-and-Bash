function GRADE()
{
 ...
 for i in `grep "((^username|^enable).*secret|^ser.*password-encryption)" system:running-config|cut -d " " -f 1`
  do if [[ -z "$i" ]]
   then (( cat1o++ )); if [[ "$test" == "TRUE" ]]; then printf '{CAT1 SV-220543r991922:\n  [service password-encryption]\n  [enable secret]}\n  {CAT2 SV-220535r960969}\n  [username secret]\n\n';
    else printf '. '
   fi
  fi
  if [[ "$i" == "service" ]]
 ...
 for i in `sh run all | grep "^ip..?ftp.(p|s)"|cut -d " " -f 3`
  do if [[ -n "$i" ]]
   then if [[ $i =~ "passive" ]]
    then (( x++ )); if [[ "$test" == "TRUE" ]]; then printf '  [ip ftp '"$i"'] : CAT1 SV-220534r960966\n';
     else printf '. '
    fi
   fi
   else 
    if [[ "$test" == "TRUE" ]]; then printf '  [ip (t|f)tp '"$i"'] : CAT1 SV-220534r960966\n';
     else printf '. '
    fi
  fi
 done
 if [[ $x != 0 ]]
  then (( cat1o = cat1o + 1 )); if [[ "$test" == "TRUE" ]]; then printf '  '"$x"' lines for 1 CAT1\n'
   else printf '. '
  fi
 fi
 ...
 if [[ `sh run | i ip_ssh_[s|c].*(sha1|ssh-rsa)` ]]
  then for i in `grep "ip.ssh.s.*(mac|encryption).*(sha1|3des)" system:running-config`
   do if [[ "$i" == "mac" ]]
    then (( cat1o++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  [ip ssh server algorithm '"$i"'] :  CAT1 SV-220555r961554\n';
     else printf '. '
    fi
   fi
   if [[ "$i" == "encryption" ]]
    then (( cat1o++ )); if [[ "$test" == "TRUE" ]]; then printf '  [ip ssh server algorithm '"$i"'] : CAT1 SV-220556r961557\n';
     else printf '. '
    fi
   fi
  done
 ...
 if [[ -z `sh run | i 900.*s.3.*120` ]]
  then (( cat2o++ )); if [[ "$test" == "TRUE" ]]; then printf '  CAT2 : SV-220524r960840\n  [login block-for 900 attempts 3 within 120] not configured\n\n';
   else printf '. '
  fi
 fi
 ...
 if [[ -z "$a" && -z "$b" ]]
  then (( cat1o++ )); if [[ "$test" == "TRUE" ]]; then printf '  [logging host (55.71.254.175 && 55.71.100.2)] not configured\n\n';
   else printf '. '
  fi
  else
    if [[ -n "$a" && -n "$b" ]]
    then printf '. '
      else (( cat1o++ )); if [[ "$test" == "TRUE" ]]; then printf '  One [logging host] not configured\n\n';
       else printf '. '
      fi
    fi
 fi
 ...
 if [[ ! `grep "^  logging enable" system:running-config` ]]
  then (( cat2o = cat2o + 7 )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  [archive\n   log config\n    logging enable] is not configured.. ouch:\n\n';
  ...
  if [[ ! `grep "^logging userinfo" system:running-config` ]]
   then (( cat2o = cat2o + 1 )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  CAT2 : SV-220526r960864\n  [archive\n   log config\n    logging enable] is configured.. but not [logging userinfo]:\n\n';
    else printf '. '
   fi
  fi
 fi
 ...
 if [[ ! `sh run | i ^ip arp.*vlan` ]]
  ...
  if [[ $z >= 9 ]]
   then printf '. '
   else (( cat2o++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  CAT2 : SV-220661r929003\n';
    else printf '. '
   fi
  fi
 fi
 ...
 if [[ $n > 0 ]]
  then (( cat3o++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  '"$n"' lines for 1 CAT3 : SV-220662r648766\n';
   else printf '. '
  fi
 fi
 ...
 for i in `sh int status|cut -d " " -f 1`
  do if [[ ! `sh run int $i|grep "description.*ACCESS"` =~ "grep" ]]
   then 
   if [[ ! `sh run int $i | i ip verify source` ]]
 ...
 for i in `sh int status|cut -d " " -f 1`
  do if [[ ! `sh run int $i|grep "description.*ACCESS"` =~ "grep" ]]
   then
    if [[ ! `sh run int $i | i block unicast` ]]
 ...
 for i in `sh int status|cut -d " " -f 1`
  do
  if [[ ! `sh run int $i|grep "description.*ACCESS"` =~ "grep" ]]
   then if [[ `sh run int $i | i open` ]]; then (( n++ )); (( x++ )); (( y++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  '"$i"' [authentication open]';
    else printf '. '
   fi
   fi
   if [[ ! `sh run int $i | i port-control auto` ]]
    then (( n++ )); (( x++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  '"$i"' [authentication port-control auto]'; 
    else printf '. '; fi;
   fi
 ...
 if [[ $n == 0 && $x == 0 && $y == 0 ]]
  then printf '\n\n  Holy shit.. good job\n'
  else (( cat1o++ )); (( kior++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  1 CAT1 : SV-220649r863283 from '"$b"' lines\n\n';
   else printf '. '
  fi
 fi
 (( a=0 )); (( b=0 )); (( c=0 )); (( g=0 )); (( i=0 )); (( n=0 )); (( s=0 )); (( t=0 )); (( v=0 )); (( x=0 )); (( y=0 )); (( z=0 )); 
 if [[ ! `sh run | i ^cla.*Out` ]]
  then (( g++ ))
   if [[ "$test" == "TRUE" ]]
 ...
 for i in `sh int status|cut -d " " -f 1`
  ...
  if [[ $a > 0 ]]
   then (( cat3o++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  '"$n"' lines for 1 CAT3 : SV-220655r917683\n';
    else printf '. '
   fi
  fi  
  if [[ $b > 0 ]]
   then (( cat2o++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  '"$n"' lines for 1 CAT2 : SV-220669r991906\n';
    else printf '. '
   fi
  fi
  if [[ $c > 0 ]]
   then (( cat2o++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  '"$n"' lines for 1 CAT2 : SV-220666r539671\n';
    else printf '. '
   fi
  fi
  if [[ $g > 0 ]]
   then (( cat2o++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  '"$n"' lines for 1 CAT2 : SV-220651r991903\n';
    else printf '. '
   fi
  fi
  if [[ $v > 0 ]]
   then (( cat2o++ )); if [[ "$test" == "TRUE" ]]; then printf '\n\n  '"$n"' lines for 1 CAT2 : SV-220672r991909\n';
    else printf '. '
   fi
  fi     
 (( n = 0 ))
